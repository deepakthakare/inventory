        ...
        <div class="card shadow mb-4">
            ...
            <form action="<?= $action; ?>" method="post">
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <?php $section_id =  json_decode(json_encode($edit_data->section_id), true);
                                $sectionArray = json_decode($section_id); ?>
                                <select name="sections_value[]" id="sections_value" placeholder="Select Warehouses" multiple>
                                    <?php foreach ($sections_list as $key => $section) { ?>
                                        <option value="<?= $section->section_id ?>" <?php if (!empty($sectionArray)) {
                                                                                        if (in_array($section->section_id, $sectionArray)) {
                                                                                            echo "selected";
                                                                                        }
                                                                                    } ?>><?= $section->name . " - " . $section->code ?></option>
                                    <?php } ?>
                                </select>
                    ...
                    <input type="hidden" name="aisle_id" value="<?php echo $edit_data->aisle_id; ?>" />
