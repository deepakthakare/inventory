            ...
            <form action="<?= $action; ?>" method="post">
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="" name="address" value="<?php echo $edit_data->address; ?>">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for="usr">Select Warehouse Places:</label>
                                <?php $place_name =  json_decode(json_encode($edit_data->place_name), true);
                                $placeArray = json_decode($place_name); ?>
                                <select name="place_value[]" id="place_value" placeholder="Select Places" multiple>
                                    <?php foreach ($places_list as $key => $place) { ?>
                                        <option value="<?= $place->place_id ?>" <?php if (!empty($placeArray)) {
                                                                                    if (in_array($place->place_id, $placeArray)) {
                                                                                        echo "selected";
                                                                                    }
                                                                                } ?>><?= $place->name . " - " . $place->code ?></option>
                                    <?php } ?>
                                </select>

                                <!-- <?php echo form_error('place_value[]') ?> -->
                            </div>
                        </div>
                    </div>
