 ...
 */
 $(document).ready(function () {
	var myTable = $("#myTable").dataTable({
		...
		iDisplayLength: 10,
		bServerSide: true,
		sAjaxSource: ADMIN_URL + "country/get_country",
		bPaginate: true,
		...
		columns: [{ data: "id" }, { data: "name" },{ data: "cust_markup" },{ data: "cop" },{ data: "cop_percentage" }],
		order: [[0, "desc"]],
		lengthMenu: [
			[10, 25, 50, 100],
		...
		columnDefs: [
			{
				render: function (data, type, row) {
					return (
						...
						"country/edit/" +
						row.id +
						'" ><i class="fas fa-pencil-alt"></i></a> <a class="btn-danger btn-circle btn-sm text-white"  data-country_id=' +
						row.id +
	...
	$("#myTable").on("click", "#btnDelete", function () {
		...
		if (confirm("Are you sure?")) {
			var url = ADMIN_URL + "country/delete";
