 ...
 */
 $(document).ready(function () {
	var myTable = $("#myTable").dataTable({
		...
		iDisplayLength: 10,
		bServerSide: true,
		sAjaxSource: ADMIN_URL + "currency/get_currency",
		bPaginate: true,
		...
		columns: [{ data: "id" }, { data: "name" },{ data: "symbol" },{ data: "exchange_rate" }],
		order: [[0, "desc"]],
		lengthMenu: [
			[10, 25, 50, 100],
		...
		columnDefs: [
			{
				render: function (data, type, row) {
					return (
						...
						"currency/edit/" +
						row.id +
						'" ><i class="fas fa-pencil-alt"></i></a> <a class="btn-danger btn-circle btn-sm text-white"  data-currency_id=' +
						row.id +
	...
	$("#myTable").on("click", "#btnDelete", function () {
		...
		if (confirm("Are you sure?")) {
			var url = ADMIN_URL + "currency/delete";
			var param = { currency_id: currency_id };
