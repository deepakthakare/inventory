 ...
 */
 $(document).ready(function () {
	var myTable = $("#myTable").dataTable({
		...
		iDisplayLength: 10,
		bServerSide: true,
		sAjaxSource: ADMIN_URL + "sections/get_sections",
		bPaginate: true,
		...
		columns: [{ data: "section_id" }, { data: "name" },{ data: "code" }],
		order: [[0, "desc"]],
		lengthMenu: [
			[10, 25, 50, 100],
		...
		columnDefs: [
			{
				render: function (data, type, row) {
					return (
						...
						"sections/edit/" +
						row.section_id +
						'" ><i class="fas fa-pencil-alt"></i></a> <a class="btn-danger btn-circle btn-sm text-white"  data-section_id=' +
						row.section_id +
	...
	$("#myTable").on("click", "#btnDelete", function () {
		...
		if (confirm("Are you sure?")) {
			var url = ADMIN_URL + "sections/delete";
			var param = { section_id: section_id };
