 ...
 */
 $(document).ready(function () {
	var myTable = $("#myTable").dataTable({
		...
		iDisplayLength: 10,
		bServerSide: true,
		sAjaxSource: ADMIN_URL + "vendors/get_vendors",
		bPaginate: true,
		...
		columns: [{ data: "id" }, { data: "name" },{ data: "code" }],
		order: [[0, "desc"]],
		lengthMenu: [
			[10, 25, 50, 100],
		...
		columnDefs: [
			{
				render: function (data, type, row) {
					return (
						...
						"vendors/edit/" +
						row.id +
						'" ><i class="fas fa-pencil-alt"></i></a> <a class="btn-danger btn-circle btn-sm text-white"  data-vendor_id=' +
						row.id +
	...
	$("#myTable").on("click", "#btnDelete", function () {
		...
		if (confirm("Are you sure?")) {
			var url = ADMIN_URL + "vendors/delete";
